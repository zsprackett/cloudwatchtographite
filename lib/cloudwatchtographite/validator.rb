...
module CloudwatchToGraphite
  ...
  class Validator
    def self.string(n)
      fail ArgumentTypeError unless n.is_a?(String)
    end

    def self.string_shorter_than(n, length)
      string(n)
      fail ArgumentLengthError unless n.length < length
    end
    ...
    def self.hash_with_key(h, key)
      fail ArgumentTypeError unless h.is_a?(Hash) && h.key?(key)
    end
    ...
    def self.hash_with_key_of_type(h, key, type)
      hash_with_key(h, key)
      fail ArgumentTypeError unless h[key].is_a?(type)
    end
  end
end
